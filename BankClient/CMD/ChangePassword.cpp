...
void ChangePassword::Execute(BankSession& session)
{
	JOutStream jos;
	// 包头命令
	uint16 cmd = CMD_CHANGE_PASSWORD;
	jos<<cmd;
	// 预留两个字节包头len(包体+包尾长度)
	...
	// 新密码
	string newpass = session.GetAttribute("newpass");
	idea.Crypt(ideaKey,(const unsigned char*)newpass.c_str(), (unsigned char *)encryptedPass, 16, true);	// 加密
	jos.WriteBytes(encryptedPass, 16);
