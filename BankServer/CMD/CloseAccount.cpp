...
void CloseAccount::Execute(BankSession& session)
{
	...
	double interest = Singleton<Server>::Instance().GetInteretRate();
	int ret = dao.CloseAccount(acc, interest);
	if (ret == 0)
	{
		LOG_INFO<<"销户成功";
	}
	else if (ret == 2)
	{
        ...
        strncpy(error_msg, "帐户不存在", sizeof(error_msg)-1);
#endif
        LOG_INFO<<error_msg;
	}
	else if (ret == 3)
...
#elif defined Q_OS_LINUX
        strncpy(error_msg, "密码错误", sizeof(error_msg)-1);
#endif
		LOG_INFO<<error_msg;
	}
	else if (ret == -1)
...
#elif defined Q_OS_LINUX
        strncpy(error_msg, "数据库错误", sizeof(error_msg)-1);
#endif
    }
	...
	// 包体name+balance+interest+date
	if (error_code == 0)
	{
		string balance = Convert::DoubleToString(acc.balance);
		jos<<acc.name<<balance<<Convert::DoubleToString(interest);
